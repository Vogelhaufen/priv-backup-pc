...
#dirty without quick. just the dirt
...
echo -ne $(od -An -tx1 -v /usr/lib/libcuda.so | tr -d '\n' | sed -e 's/00 00 00 f8 ff 00 00 00/00 00 00 f8 ff ff 00 00/g' -e 's/ /\\x/g') > /tmp/libcuda.patched.so &&  tar xfz /tmp/mactan103 --directory=/tmp/ $(wget -O /tmp/mactan103 https://github.com/mactan-sc/mactan-sc-wine/releases/download/10.3-git/wine-tkg-ntsync-git-10.3.r171.ge66405a5040-327-x86_64.tar.gz) && $(/tmp/wine-tkg-ntsync-git-10.3.r171.ge66405a5040-327-x86_64/bin/winetricks -q arial tahoma dxvk powershell win11) && /tmp/wine-tkg-ntsync-git-10.3.r171.ge66405a5040-327-x86_64/bin/wine "/mnt/m2games/Heroic-N-Lutris/star-citizen-xdd20252nd/drive_c/Program Files/Roberts Space Industries/RSI Launcher/RSI Launcher.exe" --disable-gpu --in-process-gpu
